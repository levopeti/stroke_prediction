...
class Measurement(object):
    def __init__(self, measurement_name, row_id, neurology_df, synchronizing=True, lightweight=False):
    ...
    def add_measurement_path(self, path):
        ...
        if path.split('/')[-1].split('_')[2] == 'L':
            key[0] = "left"
        elif path.split('/')[-1].split('_')[2] == 'R':
            key[0] = "right"
        else:
            raise ValueError("Bad measurement path: {}".format(path))

        if path.split('/')[-1].split('_')[3] == 'L':
            key[1] = "leg"
        elif path.split('/')[-1].split('_')[3] == 'A':
            key[1] = "arm"
        else:
            raise ValueError("Bad measurement path: {}".format(path))
    ...
    def check_five_class(self):
        for class_value in self.class_value_dict.values():
            if class_value == 5:
                return True
        self.valid = False
        print(colored("{} has no limb with class 5 (it will be deleted)".format(self.measurement_name), "red"))
        return False
    ...
    def add_aux_data(self, aux_data_df):
        ...
        # assert isinstance(self.valid_start_time, datetime.datetime), self.valid_start_time
        # assert isinstance(self.valid_end_time, datetime.datetime), type(self.valid_end_time)

        # date = [int(x) for x in start_time.split(" ")[0].split(".") if len(x) > 0]
        # time = [int(x) for x in start_time.split(" ")[1].split(":") if len(x) > 0]
        # self.valid_start_time = datetime.datetime(date[0], date[1], date[2], time[0], time[1], time[2])
        #
        # date = [int(x) for x in end_time.split(" ")[0].split(".") if len(x) > 0]
        # time = [int(x) for x in end_time.split(" ")[1].split(":") if len(x) > 0]
        # self.valid_end_time = datetime.datetime(date[0], date[1], date[2], time[0], time[1], time[2])

    def get_absolute_class_value(self):
